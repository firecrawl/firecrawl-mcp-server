...
On the keyless free tier, `scrape`, `search`, `firecrawl_developer_search`, and `parse` work without an API key (rate-limited). Other tools such as `crawl`, `map`, and `agent` still need a key.

...
On the hosted keyless free tier, `scrape`, `search`, and hosted `parse` work without an API key while eligible (rate-limited). `interact`, `crawl`, `map`, `agent`, `extract`, and monitor tools require an API key or OAuth session.
...
Rate limits are enforced by Firecrawl. Use an API key for higher limits and access to the full MCP tool set. When a request is limited or fails, the MCP server returns the API error so the client can decide whether to retry.
...
- **If you need interactive browser automation** (click, type, navigate): use **interact** with a URL for a fresh page, or **scrape** + **interact** when you already scraped the page or need tighter scrape control. `firecrawl_interact` is the supported browser-capable path; the deprecated browser tools are intentionally not restored.
...
| parse    | Hosted upload refs or self-hosted local files | markdown, JSON, or parsed document output    |
| extract  | Structured extraction from URLs     | JSON structured data                         |
| agent    | Complex multi-source research       | job ID, then structured output via status    |
...
| research | Paper and GitHub repository research | research results, paper details, and repo matches |

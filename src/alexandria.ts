...
export const ALEXANDRIA_INSTRUCTIONS =
  ...
  'Use sources: ["alexandria"] for semantic tools only, sources: ["web"] for web only, or sources: ["web"], domainTools: true for web plus domain tools. domainTools: false disables domain matching. Results in data.tools describe available tools, not executed data. toolDetail: "compact" returns only provider, capability and description; "summary" (default) adds metadata and navigation. Inspect a selected tool with firecrawl_find_tools using its provider and capability, or set toolDetail: \"full\" for contracts upfront. Reuse a complete returned contract. ' +

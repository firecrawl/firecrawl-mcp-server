...
On an authenticated session, tool matches describe available capabilities; \`firecrawl_find_tools\` returns their contracts and \`firecrawl_scrape\` with an \`alexandria\` body executes a selected capability. Optional Alexandria quality feedback is available through firecrawl_feedback with endpoint "alexandria" on authenticated sessions (free, no job ID). Keyless sessions get no Alexandria matches in data.tools.
